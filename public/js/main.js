...
if (! function_exists( 'activate_side_navigation')) {
	ish.activate_side_navigation = function ($) {
		...
		if (jQuery('.ish-sidenav').length > 0) {
			...
			// Open sidenav
			sidenav_flBtn.on('click', function () {
				ish.open_sidenav();
	...
	ish.do_text_separator_size_fix = function (){
		...
		if ( lined.length > 0 ) {
			lined.each(function() {
				...
				var textwidth = parseInt(self.textWidth(), 10) + paddingLeftPlusRight;
				var selfwidth = parseInt(self.parent().width(), 10);
...
jQuery.fn.multipleDropDown = function() {
    ...
    nav.children('li').each(function($) {
        ...
        if ( me.find('ul').length > 0  ) {
            ...
            me.children('a').on('click', function(e){
		...
		if ( acc.length > 0 ) {
			...
			acc.each(function() {
				...
				if ( _this.attr('class').indexOf("accordion") !== -1 ) {
					_this.find('.ish-tgg-acc-title').on('click', function() {
						var _t = jQuery(this);
...
if ( ! function_exists( 'activate_shortcode_alert' ) ) {
	ish.activate_shortcode_alert = function ($){
		...
		if ( alertsClose.length > 0 ) {
			...
			alertsCloseBtn.on('click', function() {
